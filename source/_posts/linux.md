...
- Linux 只有一个**根目录 (/)**, 所有文件都存放在根目录下；
  如 '/usr/local/hello.txt' 起始斜杠'/'表示根目录，后面的斜杠就是层级关系；
...
- 配置sudo认证
  1. 先切换到 root 账号，
  2. 执行 `visudo` 命令后会自动通过vim编辑器打开/etc/sudoers (或手动编辑`vim /etc/sudoers`)，
...
- rm (删除)
  -f 表示强制删除 (使用 -f 直接删除不会出现删除提示)
  -r 删除目录及目录下的所有文件
  `rm text3.txt` 删除文件(空格跟多个文件)；
...
- `useradd [-g -d] username`
  -g 指定用户组(前提是必须存在用户组)，不加-g选项默认会创建同名用户组并自动加入；
  -d 创建用户时指定用户目录，默认是 /home/username；
  `useradd -g testGroup test1 -d /home/test111`
  创建用户 test1，并加入 testGroup 用户组，并指定用户目录为 /home/test111
  `su test1`切换到 test1 用户、`pwd` 显示test1 用户目录；
- `userdel [-r] username`
  -r 删除用户 /home/username 目录，否则保留；
  test1用户需先退出登录；
...
- `echo "hello world" > text.txt`
  将 "hello world" 覆盖 text.txt 文件内容

- `echo "hello world" >> text.txt`
  将 "hello world" 追加到 text.txt 文件中

- `cat "text.txt" > text2.txt `
  将 text.txt 文件所有内容复制到 text2.txt 文件

- `cat "text.txt" >> text2.txt `
  将 text.txt 文件的内容追加到 text2.txt 文件

## vim

是 Linux 的文本编辑器

`vim text.txt` 进入文本编辑的命令模式界面，通过text.txt文件 (若 text.txt 文件不存在, 则会创建该文件)；
...
yum 命令需要 root 权限, 可通过 sudo 或 su root 切换到 root 用户；
yum -y install wget 需要先安装wget，再通过wget安装其他软件；
...
systemctl 是 Linux 的服务管理工具，可查看、启动、停止、重启服务；

- Linux 内置服务
  `firewalld` 防火墙服务；
  `NetworkManager` 主网络服务
  `network` 副网络服务
  `sshd/ssh` ssh/sshd 登录服务 (FinalShell远程登录使用的就是这个服务)
...
- `ping [-c count] ip/domain`
  ping 命令用于测试指定的网络服务器是否可联通状态；

- `wget [-b] url`
  类似于windows的迅雷下载文件一样；
  wget 命令用于下载网络文件，并把日志文件保存当前工作目录的wget-log文件中；
  -b 命令表示后台运行；

- `curl [-O] url`
  发送http请求，可获取html内容，也可用于下载文件；
  -O 仅限下载文件，当url是下载链接时，此项可保存文件；

## port

计算机之间通信，IP地址只能代表一台电脑，通过端口就能锁定这台电脑某个具体程序；
公认端口 1 - 1023之间，通常是系统内置预留使用，如22是ssh远程链接服务端口，80是http端口，443是https服务的端口；
...
- 设置环境变量
  1. 当前用户通过vim 打开 `vim ~/.bashrc` 文件 (所有用户 `vim /etc/profile`)；
