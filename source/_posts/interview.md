...
- 网络性能
  ...
  - 懒加载方式，多次拉取数据，避免一次性获取数据过多；
  - socket 方式，分批次发送；
  - fetch 流式读取，第一次等待响应头，就可以开始流式读取，不必像第二次等待所有响应体一样；但是 JSON 格式需要处理 JSON 格式处理器；

- 渲染性能

  - 分页减少 DOM；
  - 分片渲染 (劣势：无法保证页面数据同步)
  - 虚拟滚动 (看不见的地方使用占位符占据，滚到到时再加载)；
  - 绘制到 canvas (交互行为大，开发量巨大)；

## 页面上巨量任务需要执行，如何保证页面不卡顿

- requestIdleCallback 方式，分批量执行，不影响主线程，但是不能涉及到对 DOM 对象的改动和获取；
- webworker 方式，在一帧里面空闲的时间处理；
- setTimeout 方式，分批量执行，但是不能保证顺序；
